...
import { buildCreateVault, suiToMist } from "@suistody/core";
...
export function CreateVaultForm({ onClose }: { onClose: () => void }) {
  ...
  const { execute, isPending, error, lastDigest } = useSignExecute({
    invalidateKeys: [["vaults", account?.address ?? ""]],
    onSuccess: () => {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <label className="block">
            <span className="text-sm text-gray-400">Initial Deposit (SUI)</span>
            <input
              type="number"
              step="0.01"
              min="0"
        ...
        {lastDigest && (
          <div className="text-green-400 text-sm bg-green-500/10 rounded p-2">
            Vault created! Tx: {lastDigest.slice(0, 16)}...
          </div>
        )}
